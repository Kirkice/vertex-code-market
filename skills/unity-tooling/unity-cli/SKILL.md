---
name: unity-cli
description: Use for Unity Editor and project operations from the terminal: detect installed editors, inspect projects, run builds/tests, read logs, check authentication, and diagnose Unity environments. Route UPM package changes to unity-package-management; route URP, shaders, RenderGraph, and GPU work to graphics-base skills.
modeSlugs:
  - code
  - graphics
allowed-tools:
...
## Vertex routing boundary

This skill operates the Unity environment; it does not decide URP API compatibility or implement rendering code. Before a graphics change, establish `ProjectVersion.txt`, `Packages/manifest.json`, `packages-lock.json`, and the active project path. Then hand off to `../graphics-base/unity-graphics` and `../graphics-base/rendering-pipeline`. Use `unity-package-management` for UPM changes.

On Windows, use PowerShell-compatible commands and never assume Bash utilities are available.

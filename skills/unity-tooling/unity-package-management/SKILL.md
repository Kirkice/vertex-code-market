...
description: Use when inspecting or changing Unity UPM packages programmatically: identify package versions and dependencies, add/remove/upgrade packages through the Unity Package Manager Client API, and validate package resolution. Route URP API and rendering decisions to graphics-base skills.
modeSlugs:
  - code
  - graphics
allowed-tools:
...
Use this skill to establish package facts for URP version detection. After package facts are known, route rendering implementation to `../graphics-base/unity-graphics` or `../graphics-base/rendering-pipeline`. Do not infer RenderGraph APIs solely from a package name; inspect the resolved package source and existing project code.

On Windows, adapt shell snippets to PowerShell and preserve user confirmation before mutating package state.
